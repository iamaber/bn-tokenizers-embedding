...
The default 32k Unigram model encoded a repeated mixed-text Go benchmark fixture at approximately 23.3 MB/s across five runs on an Apple M5 arm64 host, with 52,928 bytes and 466 allocations per benchmark operation. The 32k BPE model measured 42.6 MB/s, with 29,120 bytes and 154 allocations per operation. A repeated-word BPE scaling benchmark measured approximately 76–79 MB/s at 1,000, 10,000, and 100,000 characters, consistent with linear scaling on this fixture after batched occurrence replacement. These are local preprocessing measurements; they are not validation-set throughput, a general Go-versus-Python claim, or encoder inference. Separate end-to-end Python measurements are in [PYTHON_API.md](PYTHON_API.md).

Round-trip validation passed for every record and every candidate (`Decode(Encode(text)) == Normalize(text)`). Vocabulary utilization is reported per bucket in the JSON reports; the 32k BPE model used 13,633 of 32,000 IDs in the Bangla validation bucket (42.6%). This is not global vocabulary coverage. The code-mixed bucket has only 156 validation examples, so its metrics have high uncertainty.
